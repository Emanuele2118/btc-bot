...
class ExecutionEngine:
    ...
    def invia_a_google_sheets(tipo_operazione, prezzo, quantita, profitto, saldo):
        ...
        try:
            ...
            print(f"Errore invio Google Sheets: {e}")
...
def esegui_ciclo():
    ...
    if puoi_operare and not azione_eseguita and lotti_attivi > 0 and prezzo_medio > 0:
        ...
        if (profitto_p >= soglia_tp) or (rsi > 74 and profitto_dollari > 0):
            ...
            ExecutionEngine.salva_portafoglio(portafoglio)
            ExecutionEngine.invia_a_google_sheets("TAKE_PROFIT", prezzo, lotto['quantita'], profitto_dollari, portafoglio["saldo_usd"])
    ...
    if not azione_eseguita and lotti_attivi > 0 and prezzo_medio > 0:
        ...
        if prezzo <= soglia_finale:
            ...
            azione_eseguita = True
            ExecutionEngine.salva_portafoglio(portafoglio)
            ExecutionEngine.invia_a_google_sheets("STOP_LOSS", prezzo, q_tot, profitto_op, portafoglio["saldo_usd"])
            ExecutionEngine.invia_telegram(messaggio)
